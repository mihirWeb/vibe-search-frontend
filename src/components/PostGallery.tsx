...
const PostsGallery: React.FC = () => {
    ...
    const loadPosts = async (page: number) => {
        try {
            ...
            // Fetch products from the new products API
            ...
            // Format products for gallery display
            const formattedPosts = data.products
                .filter((product: any) => product.image_url) // Only products with images
                .map((product: any) => ({
                    id: product.id,
                                ...
                                <div className="relative overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-shadow bg-white">
                                    {proxyImageUrl && (
                                        <img
                                            ...
                                            loading="lazy"
                                            onError={(e) => handleImageError(e, post.image_url)}
                                        />
                                    )}
                                    ...
                                    {/* Heart Icon */}
                                    <button className="absolute top-3 right-3 bg-white rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-gray-100">
